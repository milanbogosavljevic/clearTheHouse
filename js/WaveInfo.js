...
(function(){
    ...
    p._init = function () {
        ...
        const currentWave = system.CustomMethods.makeText('Wave', '30px Arial', '#fff', 'center', 'middle');
        currentWave.y = -165;
        const waveNumber = this._waveNumber = system.CustomMethods.makeText('0', '30px Arial', '#ffff36', 'center', 'middle');
        waveNumber.y = currentWave.y + spacing2;

        const enemies = system.CustomMethods.makeText('Enemies', '30px Arial', '#fff', 'center', 'middle');
        enemies.y = currentWave.y + spacing;
        const numberOfEnemies = this._numberOfEnemies = system.CustomMethods.makeText('0', '30px Arial', '#ffff36', 'center', 'middle');
        numberOfEnemies.y = enemies.y + spacing2;

        const activeEnemies = system.CustomMethods.makeText('Active Enemies', '30px Arial', '#fff', 'center', 'middle');
        activeEnemies.y = enemies.y + spacing;
        const numberOfActiveEnemies = this._numberOfActiveEnemies = system.CustomMethods.makeText('0', '30px Arial', '#ffff36', 'center', 'middle');
        numberOfActiveEnemies.y = activeEnemies.y + spacing2;

        const damage = system.CustomMethods.makeText('Enemies Damage', '30px Arial', '#fff', 'center', 'middle');
        damage.y = activeEnemies.y + spacing;
        const enemiesDamage = this._enemiesDamage = system.CustomMethods.makeText('0', '30px Arial', '#ffff36', 'center', 'middle');
